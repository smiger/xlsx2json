 ...
 * save workbook
 */
function serializeWorkbook(parsedWorkbook, dest, master_name="") {

  for (let name in parsedWorkbook) {
    let sheet = parsedWorkbook[name];
    if(name === config.xlsx.master && master_name){
      name = master_name;
    }
...
module.exports = {
  ...
  toJson: function (src, dest) {
    ...
    serializeWorkbook(parsed_workbook, dest, parsed_src.name);
